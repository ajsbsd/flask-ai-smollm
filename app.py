...
class Config:
    ...
    # FIX 1: SECRET_KEY must come from env — never regenerate on restart or
    # sessions break
...
# FIX 2: Server-side AI cooldown — keyed by IP, not session (session is
# client-controlled)
_ai_cooldowns = {}
    ...
    def decorator(f):
        ...
        def decorated(*args, **kwargs):
            if 'user_id' not in session:
                return jsonify(output="Auth Required.") if request.path.startswith(
                    '/api/') else redirect(url_for('login'))
...
def sanitize(text):
    return bleach.clean(
        text, tags=[], strip=True) if BLEACH_AVAILABLE else text
...
class CommandHandler:
    ...
    def ai(args, db):
        ...
        ctx_text = ""
        if "results" in session.get(
                'last_search', {}) and "based on" in args.lower():
            ctx_text = "\n".join(
                [f"Snippet: {r['snippet']}" for r in session['last_search']['results']])
...
def execute():
    ...
    if cmd == "help":
        return jsonify(output="Available: " +
                       ", ".join(commands.keys()) + ", clear, motd, whoami, startx")
    if cmd == "clear":
    ...
    if cmd == "motd":
        return jsonify(
            output=f"Welcome to {app.config['ORG_NAME']} Shell {app.config['VERSION']}")
    if cmd == "startx":
