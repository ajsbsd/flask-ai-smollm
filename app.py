...
class CommandHandler:
    ...
    def cat(args, ctx):
        ...
        res = ctx['db'].execute('SELECT content FROM posts WHERE id=?', (post_id,)).fetchone()
    ...
    def search(args, ctx):
        ...
        try:
            ...
            ctx['session']['last_search'] = {"results": [{"page_number": r[0], "snippet": r[1]} for r in rows]}
    ...
    def ai(args, ctx):
        ...
        if "results" in last_search and "based on" in args.lower():
            ctx_text = "\n".join([f"Snippet: {r['snippet']}" for r in last_search['results']])
    ...
    def contact(args, ctx):
        ...
        ctx['db'].execute('INSERT INTO contacts (email, message) VALUES (?, ?)', (parts[0], sanitize(parts[1])))
        ctx['db'].commit()
        return "Logged."

    @staticmethod
    def help(args, ctx):
        cmds = ["ls", "cat", "search", "ai", "clear", "music", "docs", "frames", "motd", "whoami", "startx"]
    ...
    def docs(args, ctx):
        ...
        try:
            rows = a_db.execute("SELECT id, title FROM documents ORDER BY id").fetchall()
            if not rows:
    ...
    def music(args, ctx):
        if args in ("start", "on"):
            ...
            return {"action": "stop_music", "output": "[ OK ] Ambient audio terminated."}
...
# --- LOGIN ROUTES ---
def _process_login_post(template_path):
    """Helper to handle POST logic for login routes."""
    form_token = request.form.get('csrf_token')
    session_token = session.pop('csrf_token', None)

    if not form_token or not session_token or form_token != session_token:
        logger.warning("CSRF validation failed.")
        return render_template(template_path, error="Session expired. Please try again.")
...
def player():
    slides = [
        {"start": 0, "end": 10, "image": url_for("static", filename="images/OpenBSD_frames/frame_1.jpg"), "text": "Transcript text for first 10 seconds"},
        {"start": 10, "end": 20, "image": url_for("static", filename="images/OpenBSD_frames/frame_2.jpg"), "text": "Transcript text for second 10 seconds"},
        {"start": 20, "end": 30, "image": url_for("static", filename="images/OpenBSD_frames/frame_3.jpg"), "text": "Transcript text for third 10 seconds"},
        {"start": 30, "end": 40, "image": url_for("static", filename="images/OpenBSD_frames/frame_4.jpg"), "text": "Transcript text for fourth 10 seconds"}
    ]
    return render_template("player.html", slides=slides, audio_url=url_for("static", filename="audio/OpenBSD.mp3"))
