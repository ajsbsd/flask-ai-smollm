...
class CommandHandler:
    ...
    def cat(args, ctx):
        ...
        res = (
            ctx["db"]
            .execute("SELECT content FROM posts WHERE id=?", (post_id,))
            .fetchone()
        )
...
def execute():
    ...
    ctx = {"db": get_db(), "ip": request.remote_addr, "session": dict(session)}
...
def _process_login_post(template_path):
    ...
    if not form_token or not session_token or form_token != session_token:
        ...
        return render_template(
            template_path, error="Session expired. Please try again."
        )
...
def player():
    ...
    return render_template(
        "player.html",
        slides=slides,
        audio_url=url_for("static", filename="audio/OpenBSD.mp3"),
    )
