...
def close_db(exc):
    ...
    if archive_db is not None: archive_db.close()

@app.context_processor
def inject_branding():
    return {
        'app_name': app.config['APP_NAME'], 'org_name': app.config['ORG_NAME'],
        'sys_ver': app.config['VERSION'], 'admin_email': app.config['ADMIN_EMAIL']
    }
...
def sanitize(text):
    ...
    # Fallback to basic HTML escaping if bleach is missing (prevents failing open)
...
class CommandHandler:
    ...
    @staticmethod
    def cat(args, ctx):
        if not args.strip(): return "Usage: cat [id]"
        if not args.strip().isdigit(): return "Error: ID must be a number. Usage: cat [id]"
    ...
    @staticmethod
    def search(args, ctx):
        if not args.strip(): return "Usage: search [term]\nExample: search Avakov"

        clean_args = re.sub(r'[^a-zA-Z0-9\s*]', '', args).strip()
        if not clean_args: return "Error: Search term must contain letters or numbers."

        s_db = get_archive_db()
        if not s_db: return "Error: imperium_archive.db not found. Build your index first."

        try:
            rows = s_db.execute(
                "SELECT page_number, snippet(document_pages, 1, '[', ']', '...', 10) "
                "FROM document_pages WHERE content MATCH ? LIMIT 3", (clean_args,)
            ).fetchall()

            if not rows: return f"No matches found for '{clean_args}'."
    ...
    def ai(args, ctx):
        ...
        with _ai_cooldown_lock:
            ...
            wait = 5 - (now - last)
            if wait > 0: return f"Wait {int(wait) + 1}s before next query."
    ...
    @staticmethod
    def help(args, ctx):
        cmds = ["ls", "cat", "search", "ai", "contact", "clear", "music", "motd", "whoami", "startx"]
    ...
    @staticmethod
    def music(args, ctx):
        if args in ("start", "on"): return {"action": "play_music", "output": "[ OK ] Ambient audio enabled."}
        elif args in ("stop", "off"): return {"action": "stop_music", "output": "[ OK ] Ambient audio terminated."}
