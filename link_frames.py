...
def main():
    ...
    search_term = Prompt.ask(
        "[bold yellow]Enter a unique word from the transcript to find the document[/bold yellow]\n"
    ...
    if not results:
        ...
        console.print("[dim]💡 Tip: Make sure the MP3 transcript has been ingested into imperium_archive.db first![/dim]")
        conn.close()
        return

    console.print(f"\n[green]✅ Found {len(results)} matching pages.[/green]")
    
    # Grab the first match's rowid (you can expand this to let the user pick if needed)
    doc_id = results[0][0]
    snippet = results[0][1].replace('[', '[bold bright_red]').replace(']', '[/bold bright_red]')
    ...
    frames_dir = Prompt.ask(
        "[bold yellow]Enter the directory containing the extracted frames[/bold yellow]", 
        default=default_dir
    )
    ...
    for i, frame_path in enumerate(frame_files):
        # Auto-calculate a sequential timestamp (e.g., 0.0, 2.5, 5.0, 7.5 for 4 frames)
        timestamp = round(i * (10.0 / max(total_frames - 1, 1)), 1) if total_frames > 1 else 0.0
        
        # Ensure the path is relative to 'static/' so Flask's url_for works correctly
        web_path = frame_path.replace("static/", "", 1) if frame_path.startswith("static/") else frame_path
    ...
    console.print(f"\n[bold green]🎉 Success![/bold green] Linked {len(frame_files)} frames to Document ID {doc_id}.")
    console.print(f"[bold cyan]👉 Next step: Open your web terminal and type: [bold white]frames {doc_id}[/bold white][/bold cyan]")
